"""
DeBERTa-v3-large NER for 11-domain scientific corpus.

Strategy:
- Joint multi-domain training with [DOMAIN_X] special tokens
- Optional domain-adaptive pretraining (DAPT) via MLM
- DeBERTa backbone + Linear head + CRF, with Focal Loss assist (alpha = sqrt(N/n_c))
- Layer-wise learning rate decay (LLRD) on backbone
- Stratified per-domain train/dev split (test files used as-is)
- Model selection by macro-F1 on dev (not micro)
- Per-domain x per-class evaluation on test
- Subword handling: only first subword of each word receives a label,
  the rest get -100 and are ignored by both CRF and focal loss
- BIO post-processing to ensure consistency
"""
...
class Cfg:
    # Paths
    data_dir = r"./data/ner"
    output_dir = r"./results/ner_deberta_joint"

    # 11 domains (file prefix used to locate {domain}_docs_{train|test}.json)
    domains = [
        "advance_material", "aerospace", "ai", "chemestry", "climate",
        "electronic", "energy", "genetic", "laser", "physics", "robatics",
    ]
    ...
    # Domain conditioning: prepend [DOMAIN_X] token to each sample
    # Set False to train a domain-agnostic joint model
...
def parse_combined_text(text):
    """Parse <Tag>span</Tag> annotations into BIO token+label lists."""
    pattern = r'<(.*?)>(.*?)</\1>|([^<>\s]+)'
...
def domain_token(d):
    return f"[DOMAIN_{d.upper()}]"


def load_domain_data(data_dir, domains):
    items = []
    for d in domains:
...
def stratified_split(items, dev_ratio, seed):
    ...
    by_domain_train = defaultdict(list)
    test = []
    for it in items:
        if it["split"] == "test":
            test.append(it)
        else:
            by_domain_train[it["domain"]].append(it)
    train, dev = [], []
    for d, lst in by_domain_train.items():
        lst = lst[:]
        rng.shuffle(lst)
...
def fix_bio(seq):
    """Ensure no I-X follows O or different entity."""
    fixed = []
    prev = "O"
    for tag in seq:
        if tag.startswith("I-"):
...
class NERDataset(Dataset):
    def __init__(self, items, tokenizer, tag2id, max_len, stride, with_domain=True):
        self.samples = []
        for it in items:
            tokens = list(it["tokens"])
            labels = list(it["labels"])
            if with_domain:
                tokens = [domain_token(it["domain"])] + tokens
                labels = ["O"] + labels
            ...
            for bi in range(len(enc["input_ids"])):
                ...
                for w in word_ids:
                    if w is None:
                        ...
                        label_ids.append(-100)   # subword: ignored
                    prev = w
...
class DebertaCRF(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        if labels is not None:
            ...
            crf_mask[:, 0] = True            # CRF requires first position True
...
def build_param_groups(model, cfg):
    ...
    for i, layer in enumerate(backbone.encoder.layer):
        depth = n_layers - i              # top layer depth = 1
...
def evaluate(model, loader, id2tag, device, compute_loss=False, focal_weight=0.0):
    ...
    all_true, all_pred = [], []
    flat_t, flat_p = [], []
    by_domain_true, by_domain_pred = defaultdict(list), defaultdict(list)
    by_domain_flat_true, by_domain_flat_pred = defaultdict(list), defaultdict(list)
    total_loss = 0.0
    n_batches = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(len(preds)):
                # Rebuild per-position prediction array from CRF output
                ...
                for pos, m in enumerate(crf_mask_np[i]):
                    if m:
                        ...
                        ptr += 1
                # Pick only positions with a real (non -100) label
                ...
                by_domain_true[domains[i]].append(t_seq)
                by_domain_pred[domains[i]].append(p_seq)
                by_domain_flat_true[domains[i]].extend(t_seq)
                by_domain_flat_pred[domains[i]].extend(p_seq)
    ...
    out = {
        ...
        "flat_true": flat_t,
        "flat_pred": flat_p,
        "by_domain_true": dict(by_domain_true),
        "by_domain_pred": dict(by_domain_pred),
        "by_domain_flat_true": dict(by_domain_flat_true),
        "by_domain_flat_pred": dict(by_domain_flat_pred),
    }
    if compute_loss:
...
def main():
    ...
    print("Loading data...")
    items = load_domain_data(cfg.data_dir, cfg.domains)
    train_items, dev_items, test_items = stratified_split(items, cfg.dev_ratio, cfg.seed)
    ...
    if cfg.use_domain_token:
        dom_tokens = [domain_token(d) for d in cfg.domains]
    ...
    # DAPT (uses train+dev only, never test)
    backbone_path = cfg.model_name
    if cfg.run_dapt:
        dapt_texts = []
        for it in train_items + dev_items:
            if cfg.use_domain_token:
                toks = [domain_token(it["domain"])] + it["tokens"]
            else:
                toks = it["tokens"]
    ...
    # Focal alpha = sqrt(N_total / n_c), normalized so mean(alpha)=1; alpha(O) overridden
    cnt = Counter()
    ...
    print(f"\nLoading best model (dev macro F1 = {best_micro:.4f})...")
    ...
    # Overall reports (5 reports + 4 confusion matrices)
    save_all_reports(res["all_true"], res["all_pred"],
                     res["flat_true"], res["flat_pred"],
                     cfg.output_dir, prefix="test_overall_")

    # Per-domain reports
    for d in cfg.domains:
        t = res["by_domain_true"].get(d, [])
        p = res["by_domain_pred"].get(d, [])
        ft = res["by_domain_flat_true"].get(d, [])
        fp = res["by_domain_flat_pred"].get(d, [])
        if not t:
            continue
        dom_dir = os.path.join(cfg.output_dir, f"domain_{d}")
        save_all_reports(t, p, ft, fp, dom_dir, prefix="test_")
